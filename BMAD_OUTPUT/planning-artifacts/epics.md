...
inputDocuments: ['BMAD_OUTPUT/planning-artifacts/PRD_V1.md', 'BMAD_OUTPUT/planning-artifacts/architecture.md', 'BMAD_OUTPUT/planning-artifacts/Backlog_V1.md']
project_name: ApiBuilderArchitecture
status: complete
---

# ApiBuilderArchitecture - Epics & User Stories
...
- Utilisation du starter template `chunxchun/erp-starter` (Vite, React, TS, Tailwind, Shadcn UI)
- Modèle de données Hybride (Colonnes typées pour l'indexation + JSONB pour la flexibilité)
- Authentification via Supabase Auth avec rôles dans `app_metadata` (pas dans `user_metadata`)
- Pattern API REST pour l'ingestion via Edge Functions
- Runtime Deno 2.1 Compatible pour les Edge Functions
- Structure Monorepo avec frontières strictes (Frontend / Supabase)
- Utilisation obligatoire d'une couche Mapper pour la conversion snake_case (DB) <-> camelCase (App)
- Gestion des logs d'audit via Triggers SQL vers table `validation_events`
- Gestion des états UI avec React Query (Server) et Zustand (Client)
